...
const ArabicLanguage = ({ language }) => {
  ...
  // Effect to update filtered suggestions on language change
  useEffect(() => {
    setFilteredSuggestions(language);
  }, [language]);
  ...
  // Search translations function
  const searchTranslations = async () => {
    setLoadingTranslations(true);
    try {
      const response = await dispatch(getTranslate(checkedSuggestions, "arabic"));
      if (response.success) {
  ...
  const Row = ({ index, style }) => (
    <ListItem style={style} key={index}>
      <FormControlLabel
        control={
          <Checkbox
            checked={checkedSuggestions.includes(filteredSuggestions[index].arabic)}
            onChange={(event) => handleCheckboxChange(event, filteredSuggestions[index].arabic)}
          />
        }
        label={filteredSuggestions[index].arabic}
      />
    </ListItem>
  );
